...
void setConfiguration(int config) {
  ...
  switch (config) {
    ...
    case State::INPUT_JACK_MIC:
      ...
      myTympan.setEnableStereoExtMicBias(true);   //enable mic bias on both left and right channels
      setInputGain_dB(myState.input_gain_dB);
      break;
    ...
    case State::INPUT_JACK_LINE:
      ...
      myTympan.setEnableStereoExtMicBias(false);   //by disabling this mic bias switch, it lowers cross talk between left and right inputs
      setInputGain_dB(myState.input_gain_dB);
      break;
  }
}
...
int setOutputChan(int chan) {
  ...
  switch (chan) {
    case State::OUT_LEFT:
      Serial.println("setOutputChan: Setting to LEFT...");
    ...
    case State::OUT_RIGHT:
      Serial.println("setOutputChan: Setting to RIGHT...");
      ...
      break;
    case State::OUT_BOTH:
      Serial.println("setOutputChan: Setting to BOTH...");
      outputSwitchMatrix.setInputToOutput(inputChanForSineWave,outputChanLeft);     //sine to left output
      outputSwitchMatrix.setInputToOutput(inputChanForSineWave,outputChanRight);    //sine to right output
      myState.output_chan = chan;
      break;
...
//clear the averaging so that it starts fresh
void clearCalcLevelAverages(void) {
  calcInputLevel_L.clearStates();
  calcInputLevel_R.clearStates();
}
